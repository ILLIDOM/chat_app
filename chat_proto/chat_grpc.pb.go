...
type ChatClient interface {
	JoinChannel(ctx context.Context, in *Channel, opts ...grpc.CallOption) (Chat_JoinChannelClient, error)
...
func (c *chatClient) JoinChannel(ctx context.Context, in *Channel, opts ...grpc.CallOption) (Chat_JoinChannelClient, error) {
	stream, err := c.cc.NewStream(ctx, &Chat_ServiceDesc.Streams[0], "/chatapp.Chat/JoinChannel", opts...)
	if err != nil {
		return nil, err
	}
	x := &chatJoinChannelClient{stream}
...
type Chat_JoinChannelClient interface {
	Recv() (*Message, error)
	grpc.ClientStream
}

type chatJoinChannelClient struct {
	grpc.ClientStream
}

func (x *chatJoinChannelClient) Recv() (*Message, error) {
	m := new(Message)
...
func (c *chatClient) SendMessage(ctx context.Context, opts ...grpc.CallOption) (Chat_SendMessageClient, error) {
	stream, err := c.cc.NewStream(ctx, &Chat_ServiceDesc.Streams[1], "/chatapp.Chat/SendMessage", opts...)
	if err != nil {
		return nil, err
	}
...
type ChatServer interface {
	JoinChannel(*Channel, Chat_JoinChannelServer) error
...
func (UnimplementedChatServer) JoinChannel(*Channel, Chat_JoinChannelServer) error {
...
func _Chat_JoinChannel_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ChatServer).JoinChannel(m, &chatJoinChannelServer{stream})
}

type Chat_JoinChannelServer interface {
	Send(*Message) error
	grpc.ServerStream
}

type chatJoinChannelServer struct {
	grpc.ServerStream
}

func (x *chatJoinChannelServer) Send(m *Message) error {
